...
* **Action:** Performed descriptive and inferential statistical analysis to determine linear and non-linear correlations between input variables (flow rates, temperatures) and target metrics.
* **Visualization:** Built automated distribution plots, heatmaps, and outlier detections using Matplotlib and Seaborn libraries in Python.

### 3. Predictive Modeling (Artificial Neural Networks)
* **Action:** Designed, trained, and optimized an Artificial Neural Network (ANN) architecture to predict heat transfer coefficients. Tested multiple transfer functions and hyperparameter tuning to minimize Mean Squared Error (MSE).

### 4. Business Intelligence & Dashboard Deployment
* **Action:** Developed an interactive Dashboard in Power BI/Tableau to transform the predictive model results into actionable insights. 
* **Features:** Includes dynamic filters for operational ranges, performance KPIs, and comparative charts between experimental data vs. model predictions.
